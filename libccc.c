...
void cccInit(void) {
	if (!cccInitialized) {
		...
		//cccLoadTable("ux0:/cptbl.dat", 0); //this would load all tables available, but it's done on demand
		cccInitialized = 1;
	}
}
...
int cccSJIStoUCS2(cccUCS2 * dst, size_t count, cccCode const * str) {
	...
	if (!(__table_ptr__[CCC_CP932])) cccLoadTable("ux0:/cptbl.dat", CCC_CP932);
...
int cccGBKtoUCS2(cccUCS2 * dst, size_t count, cccCode const * str) {
	if (!str || !dst) return 0;
	if (!cccInitialized) cccInit();
	if (!(__table_ptr__[CCC_CP936])) cccLoadTable("ux0:/cptbl.dat", CCC_CP936);
...
int cccKORtoUCS2(cccUCS2 * dst, size_t count, cccCode const * str) {
	if (!str || !dst) return 0;
	if (!cccInitialized) cccInit();
	if (!(__table_ptr__[CCC_CP949])) cccLoadTable("ux0:/cptbl.dat", CCC_CP949);
...
int cccBIG5toUCS2(cccUCS2 * dst, size_t count, cccCode const * str) {
	if (!str || !dst) return 0;
	if (!cccInitialized) cccInit();
	if (!(__table_ptr__[CCC_CP950])) cccLoadTable("ux0:/cptbl.dat", CCC_CP950);
	...
	switch (cp) {
		...
		default: 
			if (cp < CCC_N_CP) { //codepage in range?
				if (!cccInitialized) cccInit();
				if (!(__table_ptr__[cp]) && (cp > 0)) cccLoadTable("ux0:/cptbl.dat", cp);
