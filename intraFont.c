	...
	if (glyph->width > 0 && glyph->height > 0) {
		if (!(glyph->flags & PGF_BMP_H_ROWS) != !(glyph->flags & PGF_BMP_V_ROWS)) { //H_ROWS xor V_ROWS (real glyph, no overlay)
			...
			if (font->fileType == FILETYPE_PGF) { //for compressed pgf format
				while (i<(glyph->width*glyph->height)) {
					...
					for (j=0; (j<=((nibble<8)?(nibble):(15-nibble))) && (i<(glyph->width*glyph->height)); j++) {
						...
						if ((font->texX + xx) & 1) {
							font->texture_data[((font->texX + xx) + (font->texY + yy) * font->texWidth)>>1] &= 0x0F;
							font->texture_data[((font->texX + xx) + (font->texY + yy) * font->texWidth)>>1] |= (value<<4);
						} else {
							font->texture_data[((font->texX + xx) + (font->texY + yy) * font->texWidth)>>1] &= 0xF0;
							font->texture_data[((font->texX + xx) + (font->texY + yy) * font->texWidth)>>1] |= (value);
						}
						i++;
					}
				}
					...
					for (xx = 0; xx < glyph->width; xx++) {
						if (glyphtype & PGF_CHARGLYPH) {
							...
							if ((font->texX + (7-(xx&7)+(xx&248))) & 1) {
								font->texture_data[((font->texX + (7-(xx&7)+(xx&248))) + (font->texY + yy) * font->texWidth)>>1] &= 0x0F;
								font->texture_data[((font->texX + (7-(xx&7)+(xx&248))) + (font->texY + yy) * font->texWidth)>>1] |= (value<<4);
							} else {
								font->texture_data[((font->texX + (7-(xx&7)+(xx&248))) + (font->texY + yy) * font->texWidth)>>1] &= 0xF0;
								font->texture_data[((font->texX + (7-(xx&7)+(xx&248))) + (font->texY + yy) * font->texWidth)>>1] |= (value);
							}
            ...
            for (i = font->texX/2; i < (font->texX+glyph->width+1)/2; i++) {
                font->texture_data[i + (font->texY-1)*font->texWidth/2] = 0;
                font->texture_data[i + (font->texY+glyph->height)*font->texWidth/2] = 0;
            }
            for (i = font->texY-1; i < (font->texY+glyph->height+1); i++) {
                font->texture_data[((font->texX-1) + (i*font->texWidth))>>1] &= (font->texX & 1) ? 0xF0 : 0x0F;
                font->texture_data[((font->texX+glyph->width) + (i*font->texWidth))>>1] &= ((font->texX+glyph->width) & 1) ? 0x0F : 0xF0;
            }
...
/*static int intraFontSwizzle(intraFont *font) {
	...
	unsigned int *src = (unsigned int*) font->texture_data;
...
void intraFontUnload(intraFont *font) {
	...
	if (font->texture) vita2d_free_texture(font->texture);
	if (font->fileType == FILETYPE_PGF) {
...
void intraFontActivate(intraFont *font) {
	...
	sceGuTexImage(0, font->texWidth, font->texWidth, font->texWidth, font->texture_data);
		...
		if (char_id < font->n_chars) {
			...
			for (j = 0; j < 3; j++) {
				...
				if (glyph_id < font->n_chars) {
					...
					vita2d_draw_texture_tint_part_scale(
						font->texture,
						left + width + glyph->left*glyphscale,
			...
			if (c_index > last_c_index) {
				...
				*/
				/*vita2d_draw_texture_tint_part_scale(
					font->texture,
					left + width + shadowGlyph->left*glyphscale*64.0f/((float)font->shadowscale),
					top + height - shadowGlyph->top *glyphscale*64.0f/((float)font->shadowscale),
					shadowGlyph->x-0.25f,
					shadowGlyph->y-0.25f,
					shadowGlyph->width,
					shadowGlyph->height,
